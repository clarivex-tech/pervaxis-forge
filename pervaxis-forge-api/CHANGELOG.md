...
#### rest-api Scaffold Template — 11 Concerns Ported (`feat(templates)`)

Every service generated by Forge now includes the full production-readiness layer out of the box. New template files mirror the Forge API implementation with `{{ model.names.namespace }}` substitution:

- `Infrastructure/Middleware/` — CorrelationIdMiddleware, ExceptionHandlerMiddleware, SecurityHeadersMiddleware
- `Infrastructure/Http/` — CorrelationIdHandler, HttpLoggingHandler
- `Infrastructure/Extensions/` — ResilienceExtensions, SerilogExtensions, TracingExtensions, ValidationExtensions, VersioningExtensions
- `Infrastructure/Validation/` — ValidationEndpointFilter
- `Models/Configuration/` — ForgeResilienceOptions, ForgeResilienceClientOptions, ForgeLoggingOptions, ForgeTracingOptions
- `Program.cstemplate.sbn` — correct middleware order, health check anonymous, all 11 concerns wired
- `csproj.sbn` — 14 NuGet packages pre-added
- `appsettings.json.sbn` — Serilog, Resilience, OpenTelemetry sections with service-specific CloudWatch log group
...
- `ScribanTemplateEngine` — `model.manifest` now explicitly mapped as `ScriptObject` so all manifest properties (including `metadata`) are accessible by name in templates
- Angular `package.json.sbn` templates — replaced `metadata?.version` Scriban optional-chain syntax with `if/else` guard; the engine escapes `?.` for Angular template compatibility which broke Scriban's own null-conditional operator inside `{{ }}` blocks

### Infrastructure

- Spec: `.kiro/specs/forge-cross-cutting-concerns/` (requirements, design, tasks)
- Tests: 173/173 passing (0 failures)
