...
- [x] **1.1.5** mTLS is listed as the preferred east-west identity mechanism but no setup guidance, certificate loading code, or Kestrel TLS configuration exists in the scaffold. Add at minimum a commented Kestrel config block and a placeholder for cert resolution. ✅ *Fixed — commented `builder.WebHost.ConfigureKestrel(...)` block with mTLS options and cert-loading placeholders added to `Program.cstemplate.sbn`.*
...
- [x] **1.3.1** There is no request validation interceptor or middleware. Proto fields arrive as their zero-values (empty string, 0) if not set — callers can send blank fields with no rejection. The scaffold should include a `ValidationInterceptor` stub or guidance for using a validation library (e.g., `FluentValidation`) at the service layer. ✅ *Fixed — `ForgeValidationInterceptor.cstemplate.sbn` emitted; registered as singleton and chained before `ForgeGrpcInterceptor` on service map in `Program.cs`.*
...
- [x] **1.4.1** Security headers are applied via `context.Response.OnStarting(...)` — this runs at response flush time. Verify that gRPC trailers do not bypass this callback; some gRPC error paths flush headers differently. ✅ *Fixed — comment added in `Program.cstemplate.sbn` documenting that `OnStarting` fires on the initial headers frame only; gRPC trailers are separate and not affected. Security headers are safe and correct for HTTP/1 endpoints.*
...
- [x] **2.4.2** The retry handler has no circuit breaker. After `RetryCount` consecutive failures it continues retrying on every new request. For enterprise workloads, add a circuit breaker (open/half-open/closed states) to shed load during downstream outages. ✅ *Fixed — open/half-open/closed circuit breaker added to `ForgeRetryDelegatingHandler`; trips after 5 consecutive failures, probes after 30-second open window.*
...
- [x] **3.1.2** No `KeepAlive` settings configured for gRPC connections. Long-lived connections through NAT/load balancers silently drop without keepalives. Add `grpc.http2.min_time_between_pings_ms` and related options to the generated configuration. ✅ *Fixed — `Kestrel.Limits.Http2.KeepAlivePingDelay` and `KeepAlivePingTimeout` wired in `Program.cstemplate.sbn` from `Forge:Grpc:KeepAlive` config section emitted in `appsettings.json.sbn`.*
...
- [x] **3.2.3** The generated `DbContext` class is not registered in `Program.cs`. Teams must add the `AddDbContext<>` call themselves with no example. Emit a commented-out registration block in `Program.cs` to guide teams. ✅ *Already resolved — `Program.cstemplate.sbn` conditionally emits `AddDbContextPool<DbContext>(options => options.UseNpgsql(...).EnableRetryOnFailure(3))` when `postgresql` is selected in the manifest.*
...
- [x] **6.1.3** No mutation testing configuration (e.g., Stryker.NET). 90% line coverage with no mutation testing can still mask logical bugs. Consider emitting a Stryker configuration file as a periodic quality gate. ✅ *Fixed — `stryker-config.json.sbn` emitted with break threshold at 50% mutation score; run with `dotnet stryker` from repo root.*
- [x] **6.1.4** No test for `ForgeApiKeyAuthenticationHandler` — specifically the timing-safe comparison, Secrets Manager fallback path, and `NoResult()` vs `Fail()` behavior. This is scaffold infrastructure and should be tested in the scaffold. ✅ *Fixed — `ForgeApiKeyAuthenticationHandlerTests.cstemplate.sbn` emitted with tests for valid key, wrong key, missing key, empty key, and cache-hit path.*
- [x] **6.1.5** No test for rate limiter behavior (429 response when over limit). This is scaffold behavior and should have a scaffold-level test. ✅ *Fixed — `ForgeRateLimiterTests.cstemplate.sbn` emitted; overrides the global limiter with a 1-permit window and asserts the second request returns 429.*
